...
# launch_express.sh 
# 1. Launches EC2 with Express API Server userdata + presigned url for bitbucket repo ssh key
# 2. Using presigned url, userdata script downloads ssh key
# 3. Git clone Express API Server barebone then cleanup all ssh related variables and files
...
function cleanup(){
	echo "${RED}Express Api Server instance launch failed.${RESET}"
...
domain_name=$1
launch_express && wait_for_instance && 
...
echo "Launched Express API Server instance: $ec2_id"
exit 0