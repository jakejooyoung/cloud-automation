...
# launch_aou.sh 
# 1. Launches EC2 with API Server userdata + presigned url for bitbucket repo ssh key
# 2. Using presigned url, userdata script downloads ssh key
# 3. Git clone API Server barebone then cleanup all ssh related variables and files
...
function cleanup(){
	echo "${RED}API Server instance launch failed.${RESET}"
...
function launch_web(){
	...
	echo "Launching & Configuring API Server..."
	launch_response=$(aws ec2 run-instances \
		--image-id ami-6e165d0e \
		--count 1 --instance-type t2.micro \
		--iam-instance-profile Name="go-launcher" \
		--key-name FounderKey \
		--security-groups apiserver jkmba-ssh \
		--user-data "$(aws s3 cp s3://npgains.userdata/api.sh - | sed "s#PRESIGNED_URL#$presigned#g")")\
...
create_tag Name "$domain_name-api-apiserver" &&
create_tag "Role" "ApiServer" &&
create_tag "Type" "API" &&
create_tag "Plan" "Startup" &&
create_tag "Owner" "npgains"

echo "Launched Api Server instance: $ec2_id"
exit 0